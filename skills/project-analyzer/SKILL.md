...
从 `skills/5gc-audit-decompose-map-checklist/references/checklists_5gc.json` 读取并内嵌类别 ID（按需作为优先级向下游传递）：
...
### 3.5 输出固定格式：# Auditing Strategy Plan

在阶段3末尾，必须输出并且仅输出下述固定标题块（给下游 `go-audit` 直接粘贴读取）。
