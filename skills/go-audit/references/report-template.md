...
**重要：详细发现必须覆盖第4章发现汇总表中的所有漏洞，不得遗漏任何一个。** 发现汇总表中有多少个漏洞，此章节就必须有多少个对应的详细条目，ID和数量必须完全一致。
...
[对第4章汇总表中的每个漏洞，逐一输出以下完整模板。不得省略任何漏洞。]
...
2. **发现完整性** — 第5章详细发现必须覆盖第4章发现汇总表中的**全部**漏洞。生成报告时逐一检查：汇总表中的每个VULN-ID在第5章中都有对应的完整条目
