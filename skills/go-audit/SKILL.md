...
2. 按四个等级生成最多四个文件（如果某等级无漏洞则不生成该文件）：
   ...
   | 文件命名 | 内容 |
   |----------|------|
   | `VULN-[该等级漏洞数量]_致命.md` | 所有致命级漏洞的详细发现 |
   | `VULN-[该等级漏洞数量]_严重.md` | 所有严重级漏洞的详细发现 |
   | `VULN-[该等级漏洞数量]_一般.md` | 所有一般级漏洞的详细发现 |
   | `VULN-[该等级漏洞数量]_提示.md` | 所有提示级漏洞的详细发现 |

3. 文件命名中的数字为该等级的漏洞数量。例如有3个致命漏洞，文件名为 `VULN-3_致命.md`

4. 每个分文件的结构：
   ```
   # 详细发现 VULN-[该等级漏洞数量]_[等级]
