...
5. **详细发现** — 所有漏洞的完整分析（输出方式见下方"详细发现输出策略"）
...
详细发现有两种输出方式，根据漏洞数量自动选择：

#### 方式A：内联输出（优先）

当漏洞数量可以在一份报告中完整输出时，将所有漏洞按等级排列直接写入主报告的第5章"详细发现"中。排列顺序：致命 → 严重 → 一般 → 提示。每个漏洞包含完整的6个必填子章节。

#### 方式B：分文件输出（篇幅受限时）

当漏洞数量较多、单份报告无法完整容纳所有漏洞的详细描述时，将详细发现拆分为按等级分类的独立文件输出到 `./reports/` 目录下。

**分文件规则：**

1. 主报告的第5章写入简要说明，指引读者查看分文件：
   > 详细发现已按漏洞等级拆分为独立文件，请查阅 `./reports/` 目录下的以下文件：
   > - `VULN-001-005_致命.md`
   > - `VULN-006-012_严重.md`
   > - ...

2. 按四个等级生成最多四个文件（如果某等级无漏洞则不生成该文件）：

   | 文件命名 | 内容 |
   |----------|------|
   | `VULN-[起始ID]-[结束ID]_致命.md` | 所有致命级漏洞的详细发现 |
   | `VULN-[起始ID]-[结束ID]_严重.md` | 所有严重级漏洞的详细发现 |
   | `VULN-[起始ID]-[结束ID]_一般.md` | 所有一般级漏洞的详细发现 |
   | `VULN-[起始ID]-[结束ID]_提示.md` | 所有提示级漏洞的详细发现 |

3. 文件命名中的编号为该等级内漏洞的VULN-ID范围，例如致命级包含VULN-001到VULN-005，则文件名为 `VULN-001-005_致命.md`

4. 每个分文件的结构：
   ```
   # 详细发现 VULN-[起始ID]-[结束ID]_[等级]

   [该等级下每个漏洞的完整详细发现，使用与主报告第5章完全相同的模板，
    包含全部6个必填子章节]
   ```

5. 每个分文件中的每个漏洞同样必须包含完整的6个必填子章节（漏洞描述、漏洞代码、数据流路径、利用场景含PoC、影响、修复建议含前后对比），不得因拆分而简化

**选择哪种方式的判断：** 尝试方式A。如果在输出过程中发现篇幅确实无法容纳所有漏洞，则切换到方式B。无论哪种方式，所有漏洞都必须被完整输出，不得遗漏。
