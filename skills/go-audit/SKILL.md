...
检查是否存在 `go-vuln-lib` skill 生成的漏洞模式库。该模式库的各攻击模式分别存储为独立文件，位于 `./.opencode/skills/go-vuln-lib/vuln-lib/patterns/` 路径中。

**加载方式：**
1. 检查 `./.opencode/skills/go-vuln-lib/vuln-lib/patterns/` 目录是否存在
2. 如果存在，列出 `patterns/` 目录下的所有模式文件
3. 根据审计策略中的高优先级漏洞类别，按需加载对应的模式文件（如审计Web API时优先加载SQL注入、SSRF、认证相关的模式文件）
...
- **go-vuln-lib 漏洞模式库**（优先）— 由 `go-vuln-lib` skill 生成的漏洞模式库，各攻击模式独立存储在 `go-vuln-lib/vuln-lib/patterns/` 目录下。在阶段4.1按需加载对应的攻击模式文件。
