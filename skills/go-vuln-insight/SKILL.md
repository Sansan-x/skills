...
#### 修复代码
[**必填**。展示修复该漏洞的代码，标注来源PR或commit。如尚无修复则标注"暂无官方修复"并给出建议修复方案。]
```go
// 来源：[PR链接 / commit hash]
[实际修复代码]
```
...
#### 根因分析
[**必填**。分析漏洞产生的根本原因，包括：设计缺陷、编码错误、缺失的防御措施等。]

#### 漏洞利用
[**必填**。描述该漏洞的利用方式，包括：]
- **攻击向量**：攻击者如何触发该漏洞（网络位置、所需权限等）
- **利用条件**：触发漏洞需要满足的前提条件
- **PoC要点**：概述构造攻击载荷的关键步骤（如Issue中提供了PoC则引用）
- **影响**：成功利用后的具体影响（DoS/信息泄露/RCE等）
