...
**模式清单（必读取）**：必须 `Read` `./reports/pattern-manifest.json`。处理其中 `category == GO_AUDIT_DETECTOR` 的 `patterns`；`pattern_execution_metrics.patterns_loaded` **必须**等于该集合大小（或与中间 JSON 中 `patterns_loaded` 数组长度一致）。不得跳过 manifest；缺失文件时中止并提示先运行 `project-analyzer`。
...
3. **输出 detector 结果**：输出 `detector_findings`、`sink_candidates`（污点轨）与 `static_findings`（静态轨，可为 `[]`）供父层与编排聚合使用。
4. **输出可聚合执行证据**：输出 `pattern_execution_metrics.GO_AUDIT_DETECTOR`，用于 `must-cover-results.json` 的类别行聚合。

若策略缺失并行编排字段，应标注“并行编排不确定”，回退到单执行器模式；不得臆造映射。

### 统一数据契约（必须遵守）

`sink_candidate`（detector 输出；供父层 `trace-resolver` 污点轨）：

```yaml
id: string
category_id: string
evidence_model: taint_traceable | static_only   # 可选；省略视为 taint_traceable（与旧行为兼容）
file: path
function: func
sink: api
line: number
...
`static_finding`（detector 输出；静态轨，与 `detector_findings` / `sink_candidates` 并存）：

```yaml
id: string
category_id: string  # 如 GENERAL_GO_SECURITY_TRAPS、GO_RUNTIME 等
evidence_model: static_only
vuln_type: string
title: string
location:
  file: path
...
必须写入：`./reports/intermediate/<run_id>/go-audit-detector.json`（`run_id` 与本轮 `must-cover-results.json` 一致；`mkdir -p "./reports/intermediate/<run_id>"`）。

- 顶层 `agent` 为 `go-audit-detector`，`category_id` 为 `GO_AUDIT_DETECTOR`。
- `patterns_loaded` / `patterns_executed` / `pattern_execution[]` 与上节 metrics **数值与语义一致**；`pattern_execution` 须覆盖 manifest 中本类别**每一个** `pattern_id`（契约见 [pattern-coverage-json-schema.md](../orchestrator-audit/references/pattern-coverage-json-schema.md)）。
- **不得**省略该文件；缺失则 orchestrator **Gate-P** 失败。
...
4. **双轨归类**：**GENERAL_GO_SECURITY_TRAPS**、不变量破坏、错误处理 / recover 语义、明显不可污点化的逻辑缺陷等，**优先**写入 `static_findings`（`evidence_model: static_only`），**不必**为此类单独伪造污点 `sink_candidate`。若同一点同时存在可达用户输入污点路径，可**额外**输出 `sink_candidate` 且 `evidence_model: taint_traceable`（或省略字段）；`static_findings` 与 `sink_candidates` 不得对同一行给出互相矛盾的结论。
...
- Detector **禁止调用 MCP**。
- 每条**污点轨**发现（需要 source→sink MCP 证据）必须输出可追踪的 `sink_candidate`，且 `evidence_model` 为缺省或 `taint_traceable`。
- 仅静态轨的发现必须写入 `static_findings`，**不要**为其写入 `evidence_model: static_only` 的 `sink_candidate` 以占位 MCP（由编排聚合后供 judge 直接消费）。
- source→sink 路径追踪（阶段4.3）由父层 `trace-resolver` 对 `taint_traceable` 候选统一调用 CodeBadger MCP 完成。
