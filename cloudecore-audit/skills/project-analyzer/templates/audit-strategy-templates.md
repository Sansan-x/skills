...
**类别默认证据轨（可选一句）：** `FILE_OPS` / `SQLI` 通常以污点 MCP 轨为主；`GO_RUNTIME` 的 Panic/DoS/竞态等常进静态轨 `static_findings`；`GENERAL_GO_SECURITY_TRAPS`、错误处理与不变量类以静态轨为主。具体仍以各 agent skill 与 `evidence_model` 为准。
