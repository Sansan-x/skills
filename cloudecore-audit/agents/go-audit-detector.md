...
2. `sink_candidates`（污点轨；`evidence_model` 缺省或 `taint_traceable` 者由父层 `trace-resolver` 调用 CodeBadger MCP）
3. `static_findings`（静态轨；Panic / GENERAL_GO_SECURITY_TRAPS / 错误处理类等优先；可为 `[]`，字段契约见 `go-audit-detector` skill）
4. `coverage_backfill_metrics`
5. `pattern_execution_metrics.GO_AUDIT_DETECTOR`（供 `must-cover-results.json` 聚合）
6. 中间 JSON：`./reports/intermediate/<run_id>/go-audit-detector.json`（逐模式 `pattern_execution`；契约见 `skills/orchestrator-audit/references/pattern-coverage-json-schema.md`）

其中 `pattern_execution_metrics.GO_AUDIT_DETECTOR`（对应上文第 5 项）至少包含：
...
- 必须读取 `./reports/pattern-manifest.json` 并落盘 `go-audit-detector.json`；缺失则 **Gate-P** 失败。
