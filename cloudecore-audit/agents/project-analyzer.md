...
1. **必须包含且仅包含** `# Auditing Strategy Plan` 固定标题块（包含所有字段：审计领域/模式、漏洞类别优先级、信任边界到关键模块映射、文件/模块审查顺序与终止条件、后续 detector/judge 的 sink/source/净化检查点）。
2. **必须将审计策略计划写入文件** `./reports/audit-strategy-plan.md`（先 `mkdir -p ./reports`）。下游 `go-audit-detector`/`go-audit-judge` 将从该文件读取，不再依赖上下文传递。
