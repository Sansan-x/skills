...
5. 污点分析采用 Sink-first：先固定 `sink_point` 与 `sink_parent_function`，再调用 `codebadger` 反查 source 并获取 source→sink 路径。
6. 每个漏洞必须先调用 MCP；仅当 `timeout/error/empty` 时允许 `LLMInferred` 降级。
7. LLM 仅在 MCP 返回路径上分析净化/校验有效性；不在路径上的校验不得用于误报排除。
8. 必须输出工具证据字段：`ToolCallStatus`、`ToolQuerySummary`、`ChainCompleteness`、`ConfidenceCapReason`。
