...
消费各 detector 输出的 `sink_candidates`，**仅对污点轨候选**（`evidence_model` 字段**缺省**或显式为 `taint_traceable`）统一通过 CodeBadger MCP 执行 Sink-first 追踪（含 CPG 准备与查询；**CPG 生成可在策略允许且 MCP 确认可复用时跳过**），输出标准化 `trace_results` 与 `trace_metrics`，供 `go-audit-judge` 与编排层使用。`evidence_model: static_only` 的项**不进入** MCP 追踪主循环（见下文 **4A**）。
...
2. `sink_candidates`（来自 `file-audit` / `sqli-audit` / `go-runtime-audit` / `go-audit-detector`；**缺省 `evidence_model` 视为 `taint_traceable`**，与旧数据兼容）
...
### B. Sink-first 追踪（仅污点队列须通过 MCP）

4. 统一去重 `sink_candidates`（按 file+function+sink+line）。
5. **分流**：将去重后的列表划分为  
   - `taint_queue`：`evidence_model` 缺省或 `taint_traceable`  
   - `skip_taint_queue`：`evidence_model == static_only`（或将来等价的「显式跳过污点」标记）  
6. **仅对 `taint_queue`** 中每个候选构造 `sink_point + sink_parent_function + target_files + max_depth` 请求（字段名以 MCP 为准），**必须**通过 CodeBadger MCP 发起 source→sink 追踪/查询；仅在单次调用 `timeout/error/empty` 时允许对该候选降级 `LLMInferred`。
7. 工具结果与降级结果必须分段输出，禁止混写为同一条「工具确认链路」。
8. **`skip_taint_queue`（策略 4A，本仓库采用）**：**不**写入 `trace_results`；静态证据由编排聚合的 `must-cover-results.json.static_findings` 进入 judge。须在 `trace_metrics.trace_skipped_counts.static_only` 记录跳过条数，并在 `degradation_notes`（或等价数组）中附一句可机读说明，例如：`skipped_taint_trace: N candidates evidence_model=static_only (4A)`。
9. `trace_call_success_rate`、`trace_call_empty_rate`、`trace_call_timeout_rate`、`trace_downgrade_rate` **仅统计**针对 `taint_queue` 的真实 MCP 调用结果（若无任何 MCP 调用，分母为 0 时在 `degradation_notes` 说明，不得以伪造调用填充）。

**（可选 4B，未采用）**：若团队选择为跳过项写入占位 `trace_results`，须满足 `tool_call_status: skipped`、`chain_evidence_type: StaticReview`、`fallback_reason: evidence_model_static_only`，且**禁止** `ToolConfirmed`。
...
- `skipped_build_reused_query` 与 `reused_existing` 语义等价于「未再执行 CPG 生成且基于已有图完成查询」；实现时二选一写入即可，但须与 `reuse_policy` 一致。
- `trace_metrics.trace_skipped_counts.static_only`（schema `1.1`）：必填；与 **4A** 一致，记录未进入 MCP 的 `static_only` 候选条数。
...
- **禁止**以「复用 CPG」为由跳过针对 **`taint_queue`** 的 MCP source→sink 追踪调用；judge 的污点轨输入仍须来自 MCP 追踪结果或合规的 `LLMInferred` 降级。**非污点模型**（`static_only`）项不强制 MCP，以本契约为准。
